...
namespace MonoDevelop.Debugger.VSTextView.QuickInfo
{
	class DebuggerQuickInfoSource : IAsyncQuickInfoSource
	{
		...
		private async Task EvaluateAndShowTooltipAsync (IAsyncQuickInfoSession session, ITextView view, SnapshotPoint point, DataTipInfo debugInfo, CancellationToken cancellationToken)
		{
			...
			var start = debugInfo.Span.GetStartPoint (textBuffer.CurrentSnapshot);
			textBuffer.CurrentSnapshot.GetLineAndColumn (start, out line, out column);
			location.Column = column;
			location.Line = line;

			var end = debugInfo.Span.GetEndPoint (textBuffer.CurrentSnapshot);
			textBuffer.CurrentSnapshot.GetLineAndColumn (end, out line, out column);
